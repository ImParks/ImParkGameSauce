...
- 키는 외부 저장소(Notion/DB)에 보관하고, MCP를 통해 런타임에 가져옵니다
- `.harness/secrets/_keymap.json`: 키를 어디서 어떻게 가져오는지 매핑만 저장

#### 키 조회 우선순위
```
1. MCP 호출 (Notion/DB에서 가져오기)  ← 기본
2. 환경변수 (MCP 불가 시 폴백)
3. 사용자에게 직접 요청 (최후 수단)
```
...
```
에이전트가 API 키 필요
  → _keymap.json에서 retrieval.mcp 설정 확인
  → MCP 도구 호출 (예: Notion DB에서 조회)
  → 메모리에서만 사용, 절대 파일에 쓰지 않음
  → 스냅샷 저장 시 sk-****1234 형태로 마스킹
```

#### 보안 규칙
- MCP 호출 결과에 포함된 키 값을 파일에 쓰거나 로그에 남기지 않는다
- 키를 가져올 수 없으면 사용자에게 요청하고, 절대 임의 값을 사용하지 않는다
- `.env` 파일은 MCP 불가 시 폴백 용도로만 사용 (.gitignore 포함)
