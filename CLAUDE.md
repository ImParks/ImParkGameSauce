...
### 대용량 문서 분할 규칙
계획서, 평가 결과, 규칙, 스펙 등 **문서가 너무 길어질 경우** 파일로 분할하여 저장합니다:
```
1. 폴더 생성: .harness/{문서종류}/
2. _index.json에 분할 파일 목록과 설명 기재
3. 각 파일은 독립적으로 읽을 수 있는 단위로 분할
4. 에이전트는 _index.json만 먼저 읽고, 필요한 파일만 선택 조회
```
**적용 기준:**
- 계획서/보고서가 대화 한 턴에 담기 힘들 정도로 길 경우
- 규칙 파일이 도메인별로 분리가 자연스러운 경우 (예: global/architecture.json, global/performance.json)
- wiki 문서가 한 파일로는 컨텍스트를 과다 소모하는 경우
