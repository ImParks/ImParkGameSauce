...
### 파일 분할 규칙 (RULE-012 시리즈)
파일이 길어질 경우 **영역별로 다른 기준**으로 분할합니다:

| 영역 | 1차 분할 기준 | 줄 수 안전망 | 인덱스 |
|------|-------------|------------|--------|
| **소스 코드** | 단일 책임(SRP) | 300줄 필수 | index.ts (re-export만) |
| **하네스 문서** | 의미 단위(시스템/역할) | 150줄 검토, 300줄 필수 | _index.json |
| **규칙/설정/Def** | 도메인/카테고리 | 200줄(규칙), 500줄(Def) | _index.json |

**공통 원칙:**
```
1. 분할 시 인덱스 파일 필수 (_index.json 또는 index.ts)
2. 인덱스에 각 파일의 역할을 최대 3줄로 요약
3. 에이전트는 폴더 진입 시 항상 인덱스부터 읽고, 필요한 파일만 선택 로드
4. 전체 파일 일괄 로드 금지
```

**분할 금지 파일** (항상 전체 로드):
- `config.json`, `agents.json`, `*/_index.json`, `overview.md`

상세 규칙: `.harness/rules/global/file-splitting.json` 참조
